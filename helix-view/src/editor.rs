...
use helix_core::{
    ...
    syntax,
    syntax::config::{AutoPairConfig, IndentationHeuristic, LanguageServerFeature, SoftWrap},
...
impl Editor {
    ...
    fn launch_language_servers(&mut self, doc_id: DocumentId) {
        ...
        let (lang, path) = (doc.language_config(), doc.path().cloned());
        let config = doc.config.load();
        let root_dirs = &config.workspace_lsp_roots;

        // store only successfully started language servers
        let language_servers = lang.map_or_else(HashMap::default, |language| {
            self.language_servers
