...
use crate::{
    ...
    ui::{
        self, editor::InsertEvent, lsp::SignatureHelp, overlay::overlayed, DynamicPicker,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MappableCommand::Typable { name, args, .. } => f
                ...
                .field(args)
                .finish(),
            MappableCommand::Macro { sequence, .. } => {
                f.debug_tuple("MappableCommand").field(sequence).finish()
            }
        }
    }
}
...
fn global_search(cx: &mut Context) {
    ...
    if let Some(initial_query) = cx.register.and_then(|reg| cx.editor.registers.last(reg)) {
...
fn join_lines_impl(cx: &mut Context, select_space: bool) {
...
fn join_lines(cx: &mut Context) {
    join_lines_impl(cx, false)
}

fn join_lines_space(cx: &mut Context) {
    join_lines_impl(cx, true)
}
