...
use helix_view::{
    ...
    theme::Style,
    CodeAction, Diagnostic, Document, View,
};
...
fn diag_picker<I: IntoIterator<Item = (Uri, Vec<Diagnostic>)>>(
    cx: &Context,
    diagnostics: I,
    ...
    for (uri, diags) in diagnostics {
        ...
        for diag in diags {
            // TODO: support showing other diagnostics.
            if let Diagnostic::Lsp { inner, server_id } = diag {
                if let Some(ls) = cx.editor.language_server_by_id(server_id) {
...
pub fn code_action(cx: &mut Context) {
    impl ui::menu::Item for CodeAction {
        type Data = ();
    ...
    let Some(actions) = helix_view::code_action::find_under_primary_selection(cx.editor) else {
