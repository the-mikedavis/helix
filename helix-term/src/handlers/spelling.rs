...
use anyhow::Result;
use helix_core::{diagnostic::DiagnosticProvider, Rope, SpellingLanguage};
use helix_event::{cancelable_future, register_hook, send_blocking};
use helix_stdx::{
    rope::{Regex, RopeSliceExt as _},
    Range,
};
use helix_view::{
    editor::Severity,
...
fn load_dictionary(language: SpellingLanguage) {
    tokio::task::spawn_blocking(move || {
        ...
        // TODO: personal dictionaries should be namespaced under dictionary dirs? Under the
        // workspace? Idk...
...
fn check_text(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        for match_ in WORDS.find_iter(text.regex_input()) {
            ...
            if !dict.check(&word) {
                diagnostics.push(Diagnostic::Core(helix_core::Diagnostic {
                    range: Range {
                    ...
                    ends_at_word: true,
                    starts_at_word: true,
                    // NOTE: a word cannot be incorrect and empty according to Spellbook.
                    zero_width: false,
                    line: text.byte_to_line(match_.start()),
