...
use crate::{
    ...
    tree_sitter::{
        self, query::UserPredicate, Capture, Grammar, InactiveQueryCursor, Node, Pattern, Query,
        QueryMatch, RopeInput,
    },
                ...
                UserPredicate::Other(predicate) => {
                    ...
                    match name {
                        ...
                        "one-line?" | "not-one-line?" => {
                            ...
                            predicates.entry(pattern).or_default().one_line =
                                Some((capture, negated));
                            Ok(())
                        }
                        _ => Err(format!("unsupported predicate (#{name} ...)").into()),
                    }
                }
        ...
        for (node_id, mut capture) in added_indent_captures {
            ...
            if let IndentCaptureType::Align(_) = capture.capture_type {
                ...
                capture.capture_type = IndentCaptureType::Align(
                    text.byte_slice(anchor.start_byte() as usize..anchor.end_byte() as usize),
                );
            }
            indent_captures
                .entry(node_id)
